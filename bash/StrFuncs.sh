...
#I               JK Script Tools
#+=========================================================================
#I  $HeadURL: svn+ssh://dethdeg.dvrdns.org/svn/KScripts2/trunk/bash/StrFuncs.sh $
#+=========================================================================
...
#I  ID                   : $Id: StrFuncs.sh 53 2012-02-17 13:29:00Z dethrophes $
#I
#+=========================================================================
#</KHeader>
...
if [ -z "${__StrFuncs_sh__:-}" ]; then
  ...
  function chr_dec_echo {
	...
	function GetLinesInString2 {
		local IFS="${NewLine}"
		local -a LinesInText=(${2})
		printf -v ${1} ${#LinesInText[@]}
	}
	function GetLinesInString {
		local IFS="${NewLine}"
		local -a LinesInText=(${1})
		printf ${#LinesInText[@]}
		#echo "${1}" | wc --lines                  || true
	}
	function GetLongestLineInString {
		#local IFS="${NewLine}"
		#local -a LinesInText=(${1})
		#local LLine=0
		#for CLine in "${LinesInText}"; do
		#  if [[ ${LLine} -lt ${#CLine} ]]; then
		#    LLine=${#CLine}
		#  fi
		#done
		#echo ${LLine}
	...
	function GetLongestLineInArray {
	...
	StrFuncsRevision=$(CleanRevision '$Revision: 53 $')
	StrFuncsDescription=''
...
if [ -n "${__GenFuncs_sh_Loaded_-}" -a "${SBaseName2}" = "StrFuncs.sh" ]; then 
	...
	"
	declare -i ECnt=0
	test_FuncType_RETURN GetLinesInString2 "$(EncodeArgs 0 2  _RETURN "${Textq}" 8)" 	|| ECnt+=${?}
	test_FuncType_echo   GetLinesInString   "$(EncodeArgs 0 1 "${Textq}" 8)" 	|| ECnt+=${?}
	test_FuncType_echo   GetLongestLineInString "$(EncodeArgs 0 1 "${Textq}" 33)" 	|| ECnt+=${?}
	test_FuncType_echo   GetLongestLineInArray  "$(EncodeArgs 0 1 "${Textq}" 33)" 	|| ECnt+=${?}
